...
import founderBright2 from '../mr-bright2.jpeg';
...
const AboutUs: React.FC = () => {
  ...
  const [brightClass, setBrightClass] = useState('mr-bright');
  const brightCardRef = useRef<HTMLDivElement>(null);
  const brightAnimationRef = useRef<NodeJS.Timeout | null>(null);

  // Monitor Mr. Bright card visibility and animate the class
  useEffect(() => {
    const brightCard = brightCardRef.current;
    if (!brightCard) return;

    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          // Start the alternating animation
          if (!brightAnimationRef.current) {
            brightAnimationRef.current = setInterval(() => {
              setBrightClass(prev => prev === 'mr-bright' ? 'mr-bright2' : 'mr-bright');
            }, 3000);
          }
        } else {
          // Stop animation when out of view
          if (brightAnimationRef.current) {
            clearInterval(brightAnimationRef.current);
            brightAnimationRef.current = null;
          }
        }
      },
      { threshold: 0.5 }
    ...
    observer.observe(brightCard);

    return () => {
      observer.unobserve(brightCard);
      if (brightAnimationRef.current) {
        clearInterval(brightAnimationRef.current);
      }
    };
  ...
  return (
    <div className="min-h-screen w-screen overflow-x-hidden bg-white dark:bg-gray-900 relative transition-colors duration-300">
      ...
      <style>{`
        ...
        /* Mr. Bright image cross-fade animation */
        .mr-bright-card {
          transition: all 0.8s cubic-bezier(0.4, 0, 0.2, 1);
        }

        .mr-bright-image-container {
          position: relative;
          width: 100%;
          height: 100%;
        }
        ...
        .dark .mr-bright-image {
          border-color: rgb(31, 41, 55);
        }

        .mr-bright-image.hidden {
          opacity: 0;
          pointer-events: none;
        }

        .mr-bright-image.visible {
          opacity: 1;
          pointer-events: auto;
        }
      ...
      <nav className={`fixed top-0 left-0 right-0 w-full z-50 backdrop-blur-md shadow-xl border-b border-gray-100 dark:border-gray-800 transition-all duration-300 bg-white/95 dark:bg-gray-900/95`}>
        ...
        <div className={`fixed inset-0 z-[9999] lg:hidden transition-all duration-500 ease-in-out ${isMenuOpen
          ...
          <div className={`fixed inset-0 w-full h-full bg-white dark:bg-gray-900 transform transition-transform duration-500 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'
            }`}>
        ...
        <div className="relative z-20 container mx-auto px-4 h-full flex flex-col justify-center items-center pt-16 md:pt-0">
          <div className="text-center max-w-4xl">
            ...
            <div className="flex flex-col md:flex-row justify-center gap-10 md:gap-16 mb-10 opacity-0 animate-slide-up-fade-in delay-400">
              ...
              <div
                ref={brightCardRef}
                className={`glassmorphism rounded-2xl p-6 flex flex-col items-center transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105 mr-bright-card ${brightClass}`}
                style={{
                  transform: `perspective(1000px) rotateY(${(mousePosition.x - 0.5) * 10}deg) rotateX(${(mousePosition.y - 0.5) * -10}deg)`,
                  transition: 'all 0.3s ease-out'
                }}
              >
                <div className="relative w-64 h-80 mb-6 overflow-hidden rounded-2xl">
                  <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-red-600 to-red-500 blur-2xl opacity-50 z-0"></div>
                  <div className="mr-bright-image-container relative z-10">
                    {/* First image - mr-bright */}
                    <img
                      src={founderBright}
                      alt="Mr. Bright"
                      className={`mr-bright-image ${brightClass === 'mr-bright' ? 'visible' : 'hidden'}`}
                    />
                    {/* Second image - mr-bright2 */}
                    <img
                      src={founderBright2}
                      alt="Mr. Bright 2"
                      className={`mr-bright-image ${brightClass === 'mr-bright2' ? 'visible' : 'hidden'}`}
                    />
                  </div>
                </div>
